...
import re
# Ugly regex to parse a line
p = re.compile(r'(\S+) (\S+) (\d+) (\S+) (\S+) (\S+) (\S+) (\S+) "(\S+)" (\S+)  (\d+) (\S+) (\S+) (\S+) (\S+) (\S+) (\S+) (\S+) (\S+) (\S+) "(.*)" (\S+) (\d+) (\d+) (\S+)').findall

def stats_elements(splitted_line, pipeline):
    """
        Compute occurrences of each values for each elements
        Except the elemets listed in skip_elements.
    """
    i = -1
...
def stats_full_url(splitted_line, pipeline):
    """
        Rebuild an URL and compute occurrences
    """
...
def stats_words(splitted_line, pipeline):
    """
        Compute occurrences of each words
    """
